  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
      <div className="space-y-6">
        <PageHeader eyebrow="Assistant" title="AI console" description="Ask operational questions across reserves, stock, deliveries, and data quality. Replies cite the data sources used." />

        <section className="panel p-4">
          <form onSubmit={(e) => { e.preventDefault(); send(q); }} className="flex items-center gap-2">
            <Sparkles className="ml-1 h-4 w-4 text-brand" />
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Ask about reserves, stock risk, inbound impact…" className="h-10 border-0 bg-transparent text-sm focus-visible:ring-0" />
        ...
        <section className="space-y-4">
          {loading && <div className="panel p-5 text-sm text-ink-muted">Thinking…</div>}
          {responses.map((r) => (
            <article key={r.id} className="panel p-5 animate-fade-in">
              <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-ink-muted">Question</div>
              <p className="mt-1 text-sm text-ink">{r.question}</p>
              <div className="mt-4 text-[11px] font-semibold uppercase tracking-[0.14em] text-ink-muted">Answer</div>
              <p className="mt-1 text-sm leading-relaxed text-ink-secondary">{r.answer}</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div>
                  <SectionTitle className="!text-[10px]">Data sources used</SectionTitle>
                ...
                <div>
                  <SectionTitle className="!text-[10px]">Follow up</SectionTitle>
      ...
      <aside className={cn("space-y-3 lg:sticky lg:top-20 lg:self-start")}>
        ...
        {[
          { icon: Building2, label: "Selected client", value: "Leman Pro" },
          { icon: Boxes, label: "Selected SKU", value: "K-2650-CR" },
          { icon: Layers, label: "Selected files", value: "sales_2025_11.xlsx, inbound_dec.csv" },
        ].map((c) => (
