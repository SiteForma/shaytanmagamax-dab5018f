...
export function CoverageDistributionBarChart({ data }: { data: { bucket: string; count: number }[] }) {
  const t = useChartTokens();
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} margin={{ left: -16, right: 8, top: 8, bottom: 0 }}>
        <CartesianGrid stroke={t.grid} strokeDasharray="3 3" vertical={false} />
        <XAxis dataKey="bucket" stroke={t.axis} fontSize={11} tickLine={false} axisLine={false} />
